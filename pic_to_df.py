...
def read_image(file, config, skipped):
    ...
    im_thresh = cv2.threshold(
        im_no_shadow, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU
    )[1]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f",
        "--foldername",
        help="Specify path of folder that contain the images. Will use new_images folder if not specified.",
        default="new_images",
    )
    parser.add_argument(
        "-o",
        "--outputfile",
        help="Specify name (and destination) of the output file to be created. Will appear in base directory if location is not specified.",
        default="output.xlsx",
    )
    parser.add_argument(
        "-of",
        "--onefile",
        dest="onefile",
        help="Specify one file to convert only one image to data frame.",
    )
